...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'G_B', 'D_A', 'D_B', 'EMT', 'EMI']
        else:  # during test time, only load Gs
            self.model_names = ['G_A', 'G_B', 'EMT', 'EMI']
        ...
        self.netEMT = init_net(self.netEMT, init_type='normal', init_gain=0.02, gpu_ids=opt.gpu_ids)
        self.netEMI = init_net(self.netEMI, init_type='normal', init_gain=0.02, gpu_ids=opt.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters(), self.netEMT.parameters(), self.netEMI.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def set_input(self, input):
        ...
        # TODO transform before get real_A
        self.combine = input['A' if AtoB else 'B'].to(self.device)  # 1,139,256,256 tensor
        self.combine = self.combine.reshape([139, 256, 256])
        traj, transist = self.combine.split([11, 128], dim=0)  # [11, 256, 256];[128, 256, 256]
        pl, sd = traj.split([2, 9], dim=0)  # point&line;speed&direction tensor
        ...
        #gtimg = gtimg.reshape([3, 256, 256]).cpu()
        ...
        self.real_A = transform_A(A_img).to(self.device)
        self.real_A = torch.unsqueeze(self.real_A, 0).to(self.device)
